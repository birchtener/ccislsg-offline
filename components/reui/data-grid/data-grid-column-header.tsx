...
function DataGridColumnHeaderInner<TData, TValue>({
  ...
  const headerLabelClassName = cn(
    "text-secondary-foreground/80 inline-flex items-center gap-1 font-normal [&_svg]:opacity-60 text-[0.8125rem] leading-tight [&_svg]:size-3.5 whitespace-normal! break-words min-w-0 max-w-full",
    className,
  );

  const headerButtonClassName = cn(
    "text-secondary-foreground/80 hover:bg-secondary data-[state=open]:bg-secondary hover:text-foreground data-[state=open]:text-foreground px-1.5 font-normal min-h-6 h-auto py-1 rounded-md whitespace-normal! break-words text-left leading-tight flex items-center justify-between w-full min-w-0 max-w-full gap-1",
    className,
  );
  ...
  if (hasControls) {
    return (
      <div className="flex items-center justify-between gap-1 w-full min-w-0 max-w-full overflow-hidden">
        <DropdownMenu>
          <DropdownMenuTrigger
            render={
              <Button
              ...
              >
                {icon && icon}
                <span className="whitespace-normal break-words text-left flex-1 min-w-0 leading-tight">
                  {resolvedTitle}
                </span>
                {sortIcon}
              </Button>
        ...
        {props.tableLayout?.columnsPinnable && canPin && isPinned && (
          <Button
            size="icon-sm"
            variant="ghost"
            className="rounded-md size-6 shrink-0"
  ...
  if (canSort || (props.tableLayout?.columnsResizable && canResize)) {
    return (
      <div className="flex items-center w-full min-w-0 max-w-full overflow-hidden">
        <Button
          variant="ghost"
          className={headerButtonClassName}
          disabled={isLoading}
  ...
  return (
    <div className={headerLabelClassName}>
      {icon && icon}
      <span className="whitespace-normal break-words text-left min-w-0 leading-tight">
        {resolvedTitle}
      </span>
    </div>
  );
